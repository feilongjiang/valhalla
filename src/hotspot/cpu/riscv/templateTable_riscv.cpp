...
void TemplateTable::if_acmp(Condition cc) {
  ...
  if (Arguments::is_valhalla_enabled()) {
    ...
    // might be substitutable, test if either x10 or x11 is null
    __ andr(x12, x10, x11);
    if (cc == equal) {
      __ beqz(x12, not_taken);
    } else if (cc == not_equal) {
      __ beqz(x12, taken);
    }
...
void TemplateTable::_new() {
  ...
  __ allocate_instance(x14, x10, x13, x11, true, slow_case);
  __ j(done);
